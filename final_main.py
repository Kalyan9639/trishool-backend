    ...
    async def generate_feedback(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a strategical formal HR analyst. Your task is to provide a direct, concise feedback on a candidate's fit for a role based on their resume sections and the job's required skills. Respond ONLY with a JSON object which contains 2 keys: "ai_verdict" and "ai_feedback".
                The JSON should directly start with '{{' and end with '}}'.
                ...
                **DON'T** write the json object starting with ``` or ending with ```. Directly start with '{{' and end with '}}'.
                """),
            ])
